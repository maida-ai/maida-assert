...
def _json(data):
    ...
    try:
        value = json.loads(
            data, parse_constant=reject_constant, object_pairs_hook=unique_keys
        )
...
def _pull(repository, pr_number, login, api):
    ...
    if permission.get("permission") not in {"write", "admin"}:
        raise CommandError(
            "Acceptance requires current write access to this repository."
        )
...
def _snapshot(repository, sha, paths, api):
    ...
    if tree.get("truncated") is not False:
        raise CommandError(
            "Repository tree is incomplete; cannot verify acceptance paths."
        )
    ...
    for path in paths:
        ...
        if entry.get("mode") != "100644" or entry.get("type") != "blob":
            raise CommandError(
                f"Acceptance path must be a tracked regular file: {path}"
            )
...
def prepare_context(
    *,
    ...
    if not reason.strip() or len(reason) > 500 or any(ord(c) < 32 for c in reason):
        raise CommandError(
            "A single-line acceptance reason of at most 500 characters is required."
        )
    api = partial(_request_json, api_url=api_url, token=token)
    ...
    if files[policy] != base_files[policy]:
        raise CommandError(
            "Changed policy cannot be accepted with a baseline; review and merge policy separately."
        )
    return {
        "version": 1,
        ...
        "baseline_sha256": sha256(files[baseline]).hexdigest(),
...
def validate_context(context, api):
    ...
    for side, key in (("head", "head_sha"), ("base", "base_sha")):
        if pull[side]["sha"] != context[key]:
            raise CommandError(
                f"PR {side} changed; request acceptance again for the latest revision."
            )
    if pull["head"]["ref"] != context["head_branch"]:
        raise CommandError("PR head branch changed; request acceptance again.")
    tree, files = _snapshot(
        context["repository"],
        context["head_sha"],
        [context["baseline"], context["policy"]],
        api,
    )
    ...
    _, base_files = _snapshot(
        context["repository"], context["base_sha"], [context["policy"]], api
    )
...
def _read_artifact(directory):
    ...
    if sorted(p.name for p in directory.iterdir()) != ["acceptance.json"]:
        raise CommandError(
            "Artifact must contain only acceptance.json; extra files are refused."
        )
...
def write_artifact(
    *, context, artifact_directory, repository, run_id, run_attempt, api_url, token
):
    if (
        ...
        raise CommandError(
            "Acceptance binding is from a different repository or workflow attempt."
        )
    data, artifact = _read_artifact(artifact_directory)
    if set(artifact) != {"context", "baseline"} or artifact["context"] != context:
        raise CommandError(
            "Artifact binding does not match the trusted authorization context."
        )
    baseline = artifact["baseline"]
    if (
        not isinstance(baseline, dict)
        or not isinstance(baseline.get("summary"), dict)
        or not isinstance(baseline.get("source_run_id"), str)
        or not baseline["source_run_id"]
    ...
    if baseline["schema_version"] != previous.get("schema_version"):
        raise CommandError(
            "Baseline schema changed; update the baseline locally with review."
        )
    ...
    old = {key: value for key, value in previous.items() if key not in metadata_keys}
    new = {key: value for key, value in baseline.items() if key not in metadata_keys}
    changed = old != new
    head_sha = context["head_sha"]
    if changed:
        baseline["acceptance"] = {
            "accepted_at": datetime.now(timezone.utc).isoformat(),
            "accepted_by": context["login"],
            "reason": context["reason"],
            "source_run_id": baseline["source_run_id"],
            "source": {
                "repository": repository,
                "pull_request": {"number": context["pr_number"]},
                "commit_sha": context["head_sha"],
            },
            "previous_baseline": {
                "path": context["baseline"],
                "sha256": context["baseline_sha256"],
            },
            "policy": {
                "path": context["policy"],
            ...
            "verdict": {
                "outcome": "accepted",
                "summary": "Intentional baseline update; fresh gate results required.",
            },
        }
        created_tree = api(
            "POST",
                ...
                "tree": [
                    {
                        ...
                        "mode": "100644",
                        "type": "blob",
                        "content": json.dumps(baseline, indent=2, allow_nan=False)
                        + "\n",
                    }
                ],
            },
        ...
        commit = api(
            ...
            payload={
                "message": "chore(maida): accept baseline update",
        ...
        api(
            "PATCH",
            f"/repos/{repository}/git/refs/heads/{quote(context['head_branch'], safe='/')}",
    ...
    return {
        ...
        "head-sha": head_sha,
        "message": "Fresh gate results and configuration review are required for this head.",
    }
